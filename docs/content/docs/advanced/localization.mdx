...
## Stripe-specific localization

Stripe subscription components such as [`<SubscriptionCards />`](/docs/components/subscription-cards), [`<CurrentPlanCard />`](/docs/components/current-plan-card), and [`<SubscriptionManagementCard />`](/docs/components/subscription-management-card) use a set of Stripe‑specific keys from `AuthLocalization`.

You can override these keys globally on `<AuthUIProvider />`:
    ...
    <AuthUIProvider
      ...
      localization={{
        ...
        STRIPE_DESCRIPTION_ON_FREE_UPGRADE:
          "You are on the free plan. Upgrade to {plan} to unlock more features.",
      }}
    >
      {children}
    </AuthUIProvider>
...
Or override them for a single Stripe component:
...
export function BillingPage() {
  return (
    <SubscriptionCards
      localization={{
        STRIPE_BILLING_SETTINGS: "Abonnement et facturation",
...
Refer to the [`AuthLocalization`](/api-reference/auth-localization) reference for the full list of available Stripe‑related keys.
